...
export default class VehicleService {
  ...
  async uploadImages(ownerId, files = []) {
    ...
    const uploaded = await Promise.all(
      files.map((file) =>
        this.storageAdapter.upload(file, { folder: `vehicles/${ownerId}` })
      )
    );
    return {
  ...
  async deleteVehicle(ownerId, vehicleId) {
    ...
    if (vehicle.imagePublicIds?.length) {
      await Promise.all(
        vehicle.imagePublicIds.map((id) => this.storageAdapter.delete(id))
      );
    }
  ...
  async getVehicleByOwner(ownerId, page = 1, limit = 10) {
    ...
    const [vehicles, total] = await Promise.all([
      ...
      this.vehicleRepo.countDocuments({ ownerId: ownerId, deleted: false }),
    ]);

    const totalPages = Math.ceil(total / limit);

    return { vehicles, page, totalPages, total };
  }
